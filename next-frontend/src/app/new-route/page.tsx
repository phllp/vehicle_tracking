...
export async function searchDirections(source: string, destination: string) {
  ...
  const [sourceResponse, destinationResponse] = await Promise.all([
    fetch(`${baseUrl}/places?text=${source}`, {
      cache: "force-cache",
      next: {
        revalidate: 24 * 60 * 60 * 1000, // 24 hours
      },
    }),
    fetch(`${baseUrl}/places?text=${destination}`),
  ]);
  ...
  const directionsResponse = await fetch(
    `${baseUrl}/directions?originId=${placeSourceId}&destinationId=${placeDestinationId}`
  );
...
export async function NewRoutePage({ searchParams }: NewRoutePageProps) {
